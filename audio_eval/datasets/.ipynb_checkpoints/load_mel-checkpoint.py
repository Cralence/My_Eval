...
class MelPairedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.datalist1 = [item for item in self.datalist1 if item.endswith(".wav") or item.endswith(".mp3")]

        self.datalist2 = [os.path.join(datadir2, x) for x in os.listdir(datadir2)]
        self.datalist2 = sorted(self.datalist2)
        self.datalist2 = [item for item in self.datalist2 if item.endswith(".wav") or item.endswith(".mp3")]
...
class WaveDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.datalist = [item for item in self.datalist if item.endswith(".wav") or item.endswith(".mp3")]
